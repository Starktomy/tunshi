// =============================================================
//  单机离线模式 - 吞食天地
//  所有网络请求已禁用，激活码任意填写均通过
// =============================================================

// 伪造URL参数，绕过"必须从直播平台URL启动"的检测
var u = "offline_user_local";
var isroot = "OK";
var isEmulators = "0";
var dd = timestamp = Date.parse(new Date());
var uuid = "offline_uuid_" + dd;

// 激活码验证 - 任意填写都通过
function jhm() {
    var jhm = $gameVariables ? $gameVariables.value(150) : null;
    if (jhm && jhm.length > 0) {
        try { eval("$gameSwitches[101]=true;$gameVariables[150]='" + jhm + "';"); } catch(e) {}
    }
}
function gjjhm() {
    var jhm = $gameVariables ? $gameVariables.value(150) : null;
    if (jhm && jhm.length > 0) {
        try { eval("$gameSwitches[102]=true;"); } catch(e) {}
    }
}
function versionId() { window._versionId = '20972319'; }
function checkUuid() { /* 离线模式跳过 */ }
function checkRoot() { /* 离线模式跳过 */ }
function advise() { /* 离线模式跳过 */ }
var itemsData = [];
...
DataManager._databaseFiles = [
	{ name: '$dataActors',       src: 'Actors.json',length:0,size:0        },
	{ name: '$dataClasses',      src: 'Classes.json',length:0,size:0       },
	{ name: '$dataSkills',       src: 'Skills.json',length:0,size:0        },
	{ name: '$dataItems',        src: 'Items.json' ,length:0,size:0        },
	{ name: '$dataWeapons',      src: 'Weapons.json',length:0,size:0       },
	{ name: '$dataArmors',       src: 'Armors.json',length:0,size:0        },
	{ name: '$dataEnemies',      src: 'Enemies.json' ,length:0,size:0      },
	{ name: '$dataTroops',       src: 'Troops.json'  ,length:0,size:0      },
	{ name: '$dataStates',       src: 'States.json' ,length:0,size:0       },
	{ name: '$dataAnimations',   src: 'Animations.json' ,length:0,size:0   },
	{ name: '$dataTilesets',     src: 'Tilesets.json' ,length:0,size:0     },
	{ name: '$dataCommonEvents', src: 'CommonEvents.json',length:0,size:0  },
	{ name: '$dataSystem',       src: 'System.json'  ,length:0,size:0      },
	{ name: '$dataMapInfos',     src: 'MapInfos.json' ,length:0,size:0     }
];
...
DataManager.loadDatabase = function() {
	...
	for (var i = 0; i < this._databaseFiles.length; i++) {
		...
		this.loadDataFile(name, prefix + src, i);
	}
	if (this.isEventTest()) {
...
DataManager.loadDataFile = function(name, src, i=0) {
	var xhr = new XMLHttpRequest();
	var url = 'data/' + src;
	var urlException = ["Animations.json","Tilesets.json","System.json"];
	if (urlException.includes(src)) {
        var flag = "1";
	} else{
	    var flag = "2";
	}
	...
	xhr.onload = function() {
		if (xhr.status < 400) {
            ...
            if (flag == "1") {
                    ...
                    DataManager._databaseFiles[i].length = window[name].length;
                    var length = DataManager._databaseFiles[i].length;
                    if (length != undefined) {
                        // [离线模式] itemsData 来自本地，跳过
                    }
            }
...
DataManager.loadGlobalInfo = function() {
	...
	try {
		...
		console.error("loadGlobalInfo error:", e);
		globalInfo = [];
	}
...
DataManager.makeSavefileInfo = function() {
	...
	info.titleDefine = localStorage.getItem("lastTitle");
...
DataManager.makeSaveContents = function() {
	for (let i = 1; i < $dataWeapons.length; i++) {
		...
		if (weapon && weapon.note && weapon.note.includes('<online>')) {
	...
	for (let i = 1; i < $dataArmors.length; i++) {
		...
		if (armor  && armor.note && armor.note.includes('<online>')) {
			SceneManager.goto(Scene_Gameover);
			return;
		}	
    }
    ...
    $gameParty.members().forEach(function(member){
        var classId = member._classId;
        var className = window["$dataClasses"][classId]["name"];

        var tmpData = "";
        tmpData = "名字:"+member._name;
        tmpData += ",职业:"+className;
        tmpData += ",血量:"+member._hp;
        tmpData += ",魔法:"+member._mp;
        tmpData += ",体力上限:"+member.mmp;
        tmpData += ",力量:"+member.atk;
        tmpData += ",物防:"+member.def;
        tmpData += ",谋略:"+member.mat;
        tmpData += ",谋防御:"+member.mdf;
        tmpData += ",敏捷:"+member.agi;
        tmpData += ",运气:"+member.luk+"|";
        saveData += (tmpData);
    })

    // [单机离线模式] 存档不再上传到服务器，保留本地
    // $.ajax({ url:"...", type:"post", data:{savedata:saveData}, success:function(res){} })
...
StorageManager.saveToWebStorage = function(savefileId, json) {
	...
	var data = LZString.compressToBase64(json);
    if (debug == 0) {
	    var rs = Android.saveRPG(key,data);
        if (rs != "OK") {
            alert("保存失败请联系管理员");
        } else {
...
if (debug == 0) {
    var filePathList = Android.filePath();
    if (filePathList != "") {
        var fileDataList = filePathList.split(",");

        for(var i=0; i<fileDataList.length;i++)	{
            localStorage.setItem(fileDataList[i],"1");
        }
    }

    var globalStore = Android.readRPG("RPG 吞食天地 Global");
    if (globalStore) {
    	localStorage.setItem("RPG 吞食天地 Global",globalStore);
    }
...
function isLZStringCompressed(str) {
    const regex = /\{/; // 正则表达式，匹配 '[' 字符
    return regex.test(str); // 使用 test 方法检测字符串是否匹配正则表达式
...
StorageManager.loadFromWebStorage = function(savefileId) {
	...
	if (debug == 0) {
        var datas = Android.readRPG(key);
        if (datas == "") {
            return LZString.decompressFromBase64(data);
        } else {
            if (!isLZStringCompressed(datas)) {
                return LZString.decompressFromBase64(datas);
            } else {
                return (datas);
            }
        }
...
BattleManager.startBattle = function() {
	...
	$gameTemp.reserveCommonEvent(67);
	localStorage.setItem("autobattle","1");
};
...
PluginManager.setup = function(plugins) {
    plugins.forEach(function(plugin) {
        var switchStatic = localStorage.getItem("switchStatic");
		var switchStatic2 = localStorage.getItem("switchStatic2");
		var wuqi = localStorage.getItem("wuqi");

        if (plugin.name == "YEP_X_AnimatedSVEnemies" && switchStatic == "1") {

        } else if (plugin.name == "TunshiEquipShow" && wuqi == null) {

        } else if (plugin.name == "MOG_ActorTurnIndicator" && wuqi == null) {

        } else if (plugin.name == "YEP_X_VisualStateFX" && wuqi == null) {

        } else {
            if (plugin.name == "YEP_X_AttachAugments" && switchStatic2 == "1") {
                plugin.status = true;
            }

 ...
 * @on YES
 * @off NO
 * @desc 新しいゲームの場合、自動的にセーブスロットを決めますか？
 * NO - false     YES - true     Default: true
 * @default true
 *
 * @param ---アクションウィンドウ---
 ...
 * @param Select Help
 * @parent ---ヘルプウィンドウ---
 * @desc スロットを選択したときに表示されるヘルプテキスト
 * @default ファイルスロットを選択してください。
 *
 * @param Load Help
 * @parent ---ヘルプウィンドウ---
 * @desc ロードオプションを選択したときに表示されるヘルプテキスト。
 * @default 保存データを読み込みます。
 *
 * @param Save Help
 * @parent ---ヘルプウィンドウ---
 * @desc セーブオプションを選択したときに表示されるヘルプテキスト。
 * @default ゲームの現在の進行状況を保存します。
 *
 * @param Delete Help
 * @parent ---ヘルプウィンドウ---
 * @desc 削除オプションを選択したときに表示されるヘルプテキスト。
 ...
 * @dir audio/se/
 * @require 1
 * @desc /audio/se/ フォルダからサウンドを削除するために使用されます。
 * ファイル拡張子を含めないでください。
 * @default Damage2
 *
 * @param Delete Volume
 ...
 * @on Show
 * @off Hide
 * @desc セーブファイルにゲームのタイトルを表示しますか？
 * NO - false     YES - true
 * @default true
 *
 * @param Invalid Game Text
 * @parent ---情報ウィンドウ---
 * @desc セーブが別のゲーム用であるときに使用されるテキスト。
 * @default このセーブデータは別のゲーム用です。
 *
 * @param Empty Game Text
 * @parent ---情報ウィンドウ---
 * @desc 保存が空のときに使用されるテキスト。
 * @default 空
 *
 * @param Map Display Name
 * @parent ---情報ウィンドウ---
 ...
 * 0 - なし; 1 - 歩行グラフィック; 2 - 顔グラフィック; 3 - SVアクター
 * @default 2
 *
 * @param Party Y Position
 * @parent ---情報ウィンドウ---
 * @desc パーティーディスプレイのベースY位置
 * 式を使用できます。
 * @default this.lineHeight() + Window_Base._faceHeight
 *
 * @param Show Actor Names
 ...
 * @param Map Location
 * @parent ---表示情報---
 * @desc 「地図の場所」を分類するために使用されるテキスト。このカテゴリを使用しないでデータを中央に配置するには、空白のままにします。
 * @default
 *
 * @param Playtime
 * @parent ---表示情報---
 * @desc 「プレイ時間」を分類するために使用されるテキスト。このカテゴリを使用しないでデータを中央に配置するには、空白のままにします。
 * @default プレイ時間:
 *
 * @param Save Count
 * @parent ---表示情報---
 * @desc 「保存回数」を分類するためのテキスト。このカテゴリを使用しないでデータを中央に配置するには、空白のままにします。
 * @default 保存回数:
 *
 * @param Gold Count
 * @parent ---表示情報---
 * @desc 「ゴールドカウント」を分類するために使用されるテキスト。このカテゴリを使用しないでデータを中央に配置するには、空白のままにします。
 * @default %1:
 *
 * @param ---技術設定---
 * @default
 ...
 * @option web
 * @option auto
 * @desc セーブシステム機能をどのモードにするか指定
 * local     web     auto
 * @default auto
 *
 * @param Local Config
 * @parent ---技術設定---
 * @desc localセーブを扱うときのconfigのファイル名
 * Default: config.rpgsave
 * @default config.rpgsave
 *
 * @param Local Global
 * @parent ---技術設定---
 * @desc localセーブを扱うときのグローバルのファイル名
 * Default: global.rpgsave
 * @default global.rpgsave
 *
 * @param Local Save
 * @parent ---技術設定---
 * @desc localセーブを操作するときのゲームセーブのファイル名
 * %1 - File Slot. Default: config.rpgsave
 * @default file%1.rpgsave
 *
 * @param Web Config
 * @parent ---技術設定---
 * @desc Webセーブを扱うときのConfigのファイル名
 * %1 - Game Name. Default: RPG Config
 * @default RPG %1 Config
 *
 * @param Web Global
 * @parent ---技術設定---
 * @desc Webセーブを操作するときのグローバルのファイル名
 * %1 - Game Name. Default: RPG Global
 * @default RPG %1 Global
 *
 * @param Web Save
 * @parent ---技術設定---
 * @desc Webセーブを操作するときのゲームセーブのファイル名
 * %1 - Game Name. %2 - File Slot. Default: RPG File%1
 * @default RPG %1 File%2
 *
 * @param ---設定---
 ...
 * @on YES
 * @off NO
 * @desc セーブファイルをロードするときにロード確認ウィンドウを表示しますか？ NO - false     YES - true
 * @default true
 *
 * @param Load Text
 * @parent ---設定---
 * @desc 保存ファイルをロードするときの確認テキスト。
 ...
 * @on YES
 * @off NO
 * @desc 上書き保存時に保存確認画面を表示する？ NO - false     YES - true
 * @default true
 *
 * @param Save Text
 * @parent ---設定---
 * @desc 保存ファイルを上書きするときに表示されるテキスト。
 ...
 * @on YES
 * @off NO
 * @desc 保存ファイルを削除するときに保存確認ウィンドウを表示しますか？ NO - false     YES - true
 * @default true
 *
 * @param Delete Text
 * @parent ---設定---
 * @desc 保存ファイルを削除するときに表示されるテキスト。
 * @default この保存ファイルを削除しますか？
 *
 * @param Confirm Yes
 * @parent ---設定---
 ...
 *   あなたは表示テキスト内にテキストコードを使うことができます。
 *
 * ============================================================================
 * Technical - Save Modes
 * ============================================================================
 *
 * 制作者に'Technical'パラメータを検討することをお勧めします。
 * ゲームが「local」または「Web」モードで実行されているように
 * 強制することができます。
 * デフォルトでは 'auto'にしますが、強制モードはテスト用です。
 * テスト目的ではありますが、ゲームを'Web'モードに従ってセーブを調整したい場合、
 * ゲームを'local'専用にしてもそのままにしておくことができます。
 ...
 * この共有の問題を解決します。
 * これで、あなたのゲームは、他のRPG Maker MVゲームプレイヤーと衝突することなく、
 * 独自のアイデンティティを持ち、独自の設定を使用し、
 * グローバルに、そしてファイルを保存するようになります。
 *
 * ============================================================================
 * Changelog
 * ============================================================================
...
Yanfly.Param.SaveMaxFiles = Number(localStorage.getItem('savemaxfile')) || 60;
...
Window_SaveInfo.prototype.drawGoldCount = function(dx, dy, dw) {
    ...
    if (Yanfly.Param.SaveVocabGoldCount.length > 0) {
      ...
      var titleDefine = this._info.titleDefine;
      this.drawText(titleDefine, dx, dy+40, dw+200, 'left');
    } else {
...
function SaveDataTitle(result) {
    localStorage.setItem("lastTitle", result);
    const scene = SceneManager._scene;
    var id = scene.savefileId();
    if (Yanfly.Param.SaveConfirmSave && StorageManager.exists(id)) {
        scene.performActionSave();
    } else {
        scene.performActionSave();
    }
}
...
Scene_File.prototype.onActionSave = function() {
    ...
    var switchs = $gameSwitches.value(1601);
    if (switchs) {
        Android.showInputDialog();
    }

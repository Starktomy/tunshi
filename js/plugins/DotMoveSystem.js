...
【使用方法】
基本上只要安装就可以使用，但是通过设置以下内容可以进行更详细的控制。 

■ 移动单位的设置
在移动路线的脚本中
this.setMoveUnit(移动单位(0~1之间的小数));
通过写入，可以设置定每步的移动单位。 
例如，要将事件移动半步
this.setMoveUnit(0.5);。
...
■ 以点为单位移动到任意角度
在移动路线的脚本中
this.dotMoveByDeg(角度(0～359的整数));
通过写入，可以以点为单位向设置角度的方向移动。 

■ 以点为单位将事件向玩家的方向移动
在移动路线的脚本中
this.dotMoveToPlayer();
通过写入，可以将事件以点为单位向玩家的方向移动。 

■ 移动到指定坐标
在移动路线的脚本中
this.moveToTarget(X坐标, Y坐标);
通过写入，可以将其移向指定的坐标。 
※1 中途与墙壁等发生碰撞时，到达坐标会发生偏移。
※2 相对于指定坐标的移动是以角色的左上为原点进行的。

■ 事件接触判定的设定
在注释了事件第一页的EV页面的第一个事件命令的基础上，
通过在注释中记载以下内容，可以更详细地设定事件接触判定。
・接触宽度
<widthArea: 接触幅(0～1之间的小数)>

・接触长度
<heightArea: 接触幅(0～1之间的小数)>

当事件优先级设置为人物下方或人物上方时，
宽度的接触范围和长度的接触范围都被用作接触范围。
设置为与人物相同时，上下触摸时使用宽度的接触范围，
左右触摸时使用长度的接触范围。 
...
例如，如果要将96*96大小的角色设置为width: 2，height: 2，则:
在显示尺寸大于48*48的角色时，由于显示开始位置与实际的XY坐标不同
必须调整显示偏移。
<width: 2>
<height: 2>
<offsetX: 0.5>
...
Game_CharacterBase#calcFar(targetCharacter)
获取角色之间的距离。距离计算的原点使用中心点。

Game_CharacterBase#calcDeg(targetCharacter)
获取目标相对于角色的角度。角度计算的原点使用中心点。

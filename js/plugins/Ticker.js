 ...
 *
 *   ・道具·武器·防具的记事栏
 *   <ticker:あいうえお> : 作为项目的说明在获取/消失时显示。如果没有，则显示项目描述的第一行
 *    
 *   ・清除日志
 ...
 * 
 * @param y位置
 * @desc 如果输入y位置偏移(Pixel)10，则偏移10像素。
 * @type number
 * @default 0
 * 
 * @param x位置
 * @desc 如果输入x位置偏移(Pixel)10，则偏移10像素。
 * @type number
 * @default 0
 * 
 * @param 幻灯片动画开始位置
 ...
 * 
 * @param y位置合わせスピード
 * @desc 当有两个或更多个时，当第一个消失时调整y位置时的动画速度[pixel/frame]
 * @type number
 * @default 5
 * 
 * @param 上か下か
 ...
 * 
 * @param 状态自动显示开关
 * @desc 当此处指定的开关为 ON 时，状态改变时自动显示代码。
 * @type switch
 * @default 0
 * 
 * @param 物品/金钱自动显示开关
 * @desc 当此处指定的开关为 ON 时，当物品、武器和盔甲以及金钱增加/减少时自动显示代码。
 * @type switch
 * @default 0
 * 
 * @param 消失時にティッカーを表示
 * @desc 物品、武器、盔甲和金币消失时是否显示窗口
 * @type boolean
 * @default true
 * 
 * @param アイテム・武器防具入手時テキスト
 * @desc 这是您获得物品/武器/盔甲时的文字。
 * %1图标、%2物品名称、%3が個数、%4が説明に変わります。
 * 可以使用消息窗口的控制命令。
 * @default %1 %2 x %3 GET %4
 *
 * @param アイテム・武器防具消失時テキスト
 * @desc 物品·武器防具消失时显示的文本。
 * %1图标、%2物品名称、%3が個数、%4が説明に変わります。
 * 可以使用消息窗口的控制命令。

...
//=============================================================================
 /*:
 * @plugindesc v1.08 吸收屏障
 * @author Yanfly Engine Plugins
 *
 * @param Barrier State
 * @type state
 * @desc 如果一个战斗员有一个屏障，他将受到这个状态的影响。
 * 在0处离开，不进入任何状态。
 * @default 0
 *
 ...
 * ============================================================================
 *
 * 吸收屏障是为战斗而增加的一种新的机械装置。屏障值，一种新型的属性，
 * 为战士提供了一层保护。任何对HP造成的直接伤害都会首先对战斗员的障碍点造成，
 * 从而减轻对战斗员造成的任何实际伤害。
 * 任何剩余的伤害都会对战斗者造成伤害。
 *
 * 通过这种机制可以利用各种机制，如永久屏障、临时屏障、屏障穿透、屏障绕过等。
 * 请在下一节中详细阅读~
 *
 * ============================================================================
 * 屏障值-说明
 * ============================================================================
 *
 * 屏障值是放置在战斗者生命顶部的缓冲区。技能或物品造成的任何直接形式的伤害
 * 将首先对战斗员的屏障值造成伤害，然后再对战斗员的生命值造成伤害。让我们在
 * 下面的例子中看看力学是如何工作的：
 *
 * --- 示例1 ----
 *
 ...
 * ============================================================================
 *
 * 一些技能和项目可以拥有一个独特的特点，称为屏障穿透。
 * 屏障穿透允许百分位或单位数量的伤害通过，忽略目标的吸收屏障。
 * 一个动作的障碍穿透越多，目标的障碍点就被忽略的越多。
 *
 * --- 示例 ---
 *
 ...
 * ============================================================================
 *
 * 有两种类型的吸收屏障：永久屏障和临时屏障。永久屏障不会在战斗过程中失效。
 * 他们获得的屏障值，如果保持不变，将保持该值。另一方面，临时屏障将持续一定数
 * 量的回合。在用户的再生阶段，当回合数达到0时，屏障值就会被去除。
 *
 * --- 示例 ---
 *
 ...
 * ---
 *
 * 那么，造成伤害时，屏障值是如何吸收的呢？伤害总是施加到最低的回合，然后是
 * 下一个最低的回合，等等，直到它达到最高点。达到最高点后，对未过期的关卡点
 * 造成伤害。例如：
 *
 * --- 示例 ---
 *
 ...
 * ============================================================================
 *
 * 对于那些精通JavaScript的人来说，你可以利用这些注释标签来允许你的技能和物品
 * 给战斗者定制屏障值。
 *
 * 技能和物品标签：
 *
 ...
 *   - 使战斗者获得等同于“value”的屏障值，持续一定数量的“回合”。
 *   如果回合数0，则该值将是临时屏障值总数。
 *
 ...
 *
 * Version 1.04:
 * - <User Barrier:-x>、<Target Barrier:-x>、<User Barrier x Turns:-y>和
 * <Target Barrier x Turns:-y>注释标签已修改。他们现在也将显示弹出窗口。
 * 如果使用<Target Barrier x Turns:-y>，它将移除y Barrier点直到x Turns。
 *
 * Version 1.03:
 * - 为RPG Maker MV 1.1.0版更新。
 *
 * Version 1.02:
 * - 修复了一个错误，如果屏障穿透率为100%，则视为0%。
 * - 增加了“屏障状态”参数。如果战场上有屏障值，这个参数将被动地将某个状态
 * 应用到战场上。这可以通过将此插件参数值保留为0来关闭。
 *
 * Version 1.01:
 * - 修正了一个在战斗开始时阻止获得屏障值的错误。
 * - 当一个使用者给予自己一个吸收屏障时，该使用者将额外获得一个回合让屏障
 *   保持不动，这样它就不会立即溶解。
 *    
 * Version 1.00:
 * - 插件完成！

...
　　var Moghunter = Moghunter || {};
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneEquip');
	...
	Moghunter.scEquip_SlotWindowX = Number(Moghunter.parameters['平移-装备槽 X'] || 448);
	Moghunter.scEquip_SlotWindowY = Number(Moghunter.parameters['平移-装备槽 Y'] || 157);
	...
	Moghunter.scEquip_SlotLayoutX = Number(Moghunter.parameters['平移-装备槽布局 X'] || 26);
	Moghunter.scEquip_SlotLayoutY = Number(Moghunter.parameters['平移-装备槽布局 Y'] || 16);
	Moghunter.scEquip_SlotWindow_width = Number(Moghunter.parameters['装备槽宽度'] || 365);
	Moghunter.scEquip_SlotWindow_height = Number(Moghunter.parameters['装备槽高度'] || 427);
	...
	Moghunter.scEquip_ItemWindowX = Number(Moghunter.parameters['平移-装备列表 X'] || 860);
	Moghunter.scEquip_ItemWindowY = Number(Moghunter.parameters['平移-装备列表 Y'] || 112);
	...
	Moghunter.scEquip_ItemWindow_height = Number(Moghunter.parameters['装备列表高度'] || 330);
...
///// edit start
Scene_Equip.prototype.updateTouchScreen = function() {
	if (TouchInput.isTriggered()) {this.checkTouchOnSprites()};
};

Scene_Equip.prototype.checkTouchOnSprites = function() {
	 console.error("checkTouchOnSprites",TouchInput.x,TouchInput.y)
		var x  = TouchInput.x ;
	 	var y = TouchInput.y ;



	if (x>305 && x< 455 && y<96 &&y>60) {
		this._commandWindow.select(0)
		this._commandWindow.callHandler("equip")
	}


	if (x>504 && x < 655 && y<96 &&y>60) {
		this._commandWindow.select(1)
		this._commandWindow.callHandler("optimize")
	}

	if (x > 693 && x < 844 && y<96 &&y>60) {
		this._commandWindow.select(2)
		this._commandWindow.callHandler("clear")
	}

 ...
 * @parent ---画像 1---
 * @type common_event
 * @desc このIDの画像がクリックされた時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 1 Repeat
 * @parent ---画像 1---
 * @type common_event
 * @desc このIDの画像が6フレーム毎に保持されている時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 1 Hold
 * @parent ---画像 1---
 * @type common_event
 * @desc このIDの画像がクリックされて保持されている時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 1 Release
 * @parent ---画像 1---
 * @type common_event
 * @desc このIDの画像が放された時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param ---画像 2---
 * @default
 ...
 * @type common_event
 * @desc 
 * このIDの画像がクリックされた時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 2 Repeat
 * @parent ---画像 2---
 ...
 * @type common_event
 * @desc 
 * このIDの画像が6フレーム毎に保持されている時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 26 Hold
 * @parent ---画像 26---
 * @type common_event
 * @desc 
 * このIDの画像がクリックされて保持されている時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param Picture 26 Release
 * @parent ---画像 26---
 * @type common_event
 * @desc 
 * このIDの画像が放された時、このコモンイベントを実行します。コモンイベントが発生しないようにするには、0に
 * @default 0
 *
 * @param ---画像 27---
 * @default
 ...
 * ============================================================================
 *
 * このプラグインを使用すると、画像をクリックまたはタッチした時、
 * 関連付けされているコモンイベントを使用可能になります。
 *
 * ============================================================================
 * Instructions
 * ============================================================================
 *
 * プラグインのパラメータでは、
 * 1から100までにリストされている各画像IDにコモンイベントIDを設定できます
 * （RPGツクールMVで使用される画像の最大数は100です）。
 * 画像の不透明度が1の場合、その画像はコモンイベントに使用できます。
 * コモンイベントと結び付けることができる4つの条件タイプがあります。
 *
 * ---
 *
 * - クリック -
 * 画像がクリックされ、表示されているコモンイベント値が0を超えると、
 * コモンイベントがトリガーされます。
 * これは、プレーヤーがマウスを離すまで
 * （または画面へのタッチが止まるまで）1回だけトリガーされます。
 *
 * - REPEAT -
 * マウスをクリックして押したままにした場合
 * （またはタッチスクリーンを押したままにしている場合）、
 * マウスが画像領域内にあり、
 * 表示されているコモンイベント値が0より上であれば、
 * プレーヤーがマウスを放す（または画面に触れるのを止める）か、
 * マウスを画像の境界の外に移動します。
 *
 * - HOLD -
 * マウスをクリックして押したままにした場合
 * （またはタッチスクリーンを押したままにしている場合）、
 * マウスが画像領域内にあり、
 * 表示されているコモンイベント値が0を超えている限りマウスを押す
 * （または画面に触れないようにする）か、
 * マウスを画像の境界線の外側に移動します。
 *
 * - RELEASE -
 * クリックされたマウスを放すと
 * （またはプレーヤーが画面へのタッチをやめた瞬間に）
 * この画像の上でマウスを放します。
 * 一覧に表示されている画像のコモンイベント値が0を超えると、
 * 解放されます。
 *
 * ---
 * 
 * コモンイベントに関連付けられた画像に関して、
 * いくつかの行動の変化があることに注意してください。
 * コモンイベントに関連付けされている画像は、
 * 全てマップのスプライトセットとは別になり、
 * 代わりにシーンに関連付けされます。
 * これは、
 *   画面がスクロールされたり、
 *   ズームインされたり、
 *   揺れたりした場合に、
 * 関連付けられた画像に影響を与えないようにするためです。
 *
 * 画像は一般にUI属性として使用されるためであり、
 * UI属性は位置を変更しないという慣習のもとで最も効果的に機能します。
 *
 * ============================================================================
 * Plugin Commands
 * ============================================================================
 *
 * 以下は、PictureCommonEvents.js で制御を追加したい場合に
 * 使用できるプラグインコマンドです。
 *
 * プラグインコマンド
 *
 *   EnableTouchMove
 *   DisableTouchMove
 *   - プレーヤーが画面上の目的地を押してそこに移動することを許可するか、
 *   プレーヤーがタッチ移動を使用できないようにします。
 *   
 *   HidePictureCommonEvents
 *   ShowPictureCommonEvents
 *   - 手動でコモンイベントに関連付けされているすべての画像を
 *   非表示/表示します。
 *   表示されている場合、画像は以前の不透明度の値に戻ります。
 *   非表示の画像はコモンイベントのトリガーにはできません。
 *
 *   MovePlayer Down
 *   MovePlayer Left
 *   MovePlayer Right
 *   MovePlayer Up
 *   - （キーボードまたはコントローラを介して）
 *   4方向入力を使用して移動しているかのように、
 *   プレーヤのキャラクタを移動させます。
 *   これは'Press'で最もよく使われます。
 *
 *   TriggerButton Ok
 *   TriggerButton Cancel
 *   TriggerButton Dash
 ...
 *   TriggerButton Right
 *   TriggerButton Down
 *   - 元の機能のすべてのキーにコモンイベントが関連付けされている場合、
 *   ゲームがそれらの元の機能の1つのボタンコマンドをトリガーするように
 *   シミュレートします。
 *
 * ============================================================================
 * Changelog

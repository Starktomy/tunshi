 ...
 * -----------------------------------------------------------------------------
 * ----插件说明
 * 1.该插件不影响rmmv设置的两个背景图片，rmmv配置的背景将放在最底层。
...
// * 插件指令
//=============================================================================
var _drill_battle_backgrounds_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	_drill_battle_backgrounds_pluginCommand.call(this, command, args);
	
	if (command === '>创建战斗背景') { // >创建战斗背景 : 1 : B数字阵列 : 1 : 上层 : -1.0 : 1.0 : 0.60
		if(args.length == 14){
			var index = Number(args[1]);
			$gameSystem._drill_battle_backgrounds_data[index] = {};								//背景id
			$gameSystem._drill_battle_backgrounds_data[index]['src_img'] = String(args[3]);		//背景资源名
			$gameSystem._drill_battle_backgrounds_data[index]['zIndex'] = Number(args[5]);		//背景图片层级
			$gameSystem._drill_battle_backgrounds_data[index]['area_index'] = String(args[7]);	//背景菜单层级
			$gameSystem._drill_battle_backgrounds_data[index]['x_speed'] = Number(args[9]);		//背景 X速度
			$gameSystem._drill_battle_backgrounds_data[index]['y_speed'] = Number(args[11]);	//背景 Y速度
			$gameSystem._drill_battle_backgrounds_data[index]['rate'] = Number(args[13]);		//背景位移比
		}
	}
	if (command === '>清空战斗背景') {
	...
	if (command === '>战斗背景') { // >战斗背景 : A : 变色调 : H : I : J1 : J2 : J3 : J4
		if(args.length >= 8){
			...
			if( SceneManager._scene.constructor.name === "Scene_Battle" ){		//区别战斗外，战斗中的开始时间
				changing['start'] = Number(args[5]) + ($gameSystem._drill_battle_backgrounds_timer || 0);
			}
			$gameSystem._drill_battle_backgrounds_changing.push(changing);
		}
	}
};
//==============================
// * 插件设置存储准备
//==============================
var _drill_battle_background_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	_drill_battle_background_sys_initialize.call(this);
    this._drill_battle_backgrounds_data = [];
    this._drill_battle_backgrounds_changing = [];
    this._drill_battle_backgrounds_timer = 0;
};

//=============================================================================
// ** 从 Spriteset_Battle 中进行背景追加
//=============================================================================
...
// ** 菜单后面层（在敌人、角色战斗图之前放置）
//==============================
var _drill_battle_background_createBattleback = Spriteset_Battle.prototype.createBattleback;
Spriteset_Battle.prototype.createBattleback = function() {    
	_drill_battle_background_createBattleback.call(this);
...
// * 创建背景
//==============================
var _drill_battle_background_createLowerLayer = Spriteset_Battle.prototype.createLowerLayer;
Spriteset_Battle.prototype.createLowerLayer = function() {
    _drill_battle_background_createLowerLayer.call(this);
	
	this._drill_sprite_backgrounds = [];		//数据初始化
	this._drill_sprite_backgrounds_data = [];
	$gameSystem._drill_battle_backgrounds_timer = 0;
	...
	for (var i = 0; i < $gameSystem._drill_battle_backgrounds_data.length; i++) {
		if( $gameSystem._drill_battle_backgrounds_data[i] != null ){
			var temp_sprite_data = JSON.parse(JSON.stringify( $gameSystem._drill_battle_backgrounds_data[i] ));	//拷贝object（杜绝引用造成的修改）
			var temp_sprite = new TilingSprite(ImageManager.loadBattleback1(temp_sprite_data['src_img']));	//TilingSprite平铺图层
			...
			this._drill_sprite_backgrounds.push(null);
			this._drill_sprite_backgrounds_data.push(null);
		}
	}
	//alert( JSON.stringify( $gameSystem._drill_battle_backgrounds_changing) );
...
// * 刷新背景
//==============================
var _drill_battle_background_update = Spriteset_Battle.prototype.update;
Spriteset_Battle.prototype.update = function() {
	_drill_battle_background_update.call(this);
	
	this.drill_updateBackgrounds();
};

Spriteset_Battle.prototype.drill_updateBackgrounds = function() {
	$gameSystem._drill_battle_backgrounds_timer += 1;
	for (var i = 0; i < this._drill_sprite_backgrounds.length; i++) {
		if( this._drill_sprite_backgrounds[i] != null ){
			var temp_sprite_data = this._drill_sprite_backgrounds_data[i];
			var temp_sprite = this._drill_sprite_backgrounds[i];
			var b_timer = $gameSystem._drill_battle_backgrounds_timer;
			...
			//变化-每帧开始
			for (var j = 0; j < $gameSystem._drill_battle_backgrounds_changing.length; j++) {
				var t = $gameSystem._drill_battle_backgrounds_changing[j];
				if( t['id'] == i ){
			...
			//4.变化-变化结束生命周期
			for(var k = $gameSystem._drill_battle_backgrounds_changing.length -1 ; k >= 0 ; k--){
				var temp_changing = $gameSystem._drill_battle_backgrounds_changing[k];
				if( temp_changing['destroy'] ){
					$gameSystem._drill_battle_backgrounds_changing.splice(k, 1);
				}
			}
		}
	};

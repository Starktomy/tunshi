...
/*:
 * @plugindesc ver1.05/战斗记录。
 * @author Yana
 * 
 * @param Stack Log UseLog
 * @text 添加记录命令
 * @desc 是在队伍命令中添加记录的命令的设置。
 * 用true/false设置。
 * @default true
 * 
 ...
 * @text 战斗开始
 * @desc 要在记录窗口中显示的战斗开始文本。
 * @default ------战斗开始------
 * 
 * @param Stack Log TurnEnd
 * @text 回合结束
 * @desc 要在记录窗口中显示的回合结束的文本。
 * %d转换为回合数。
 * @default ------ 第%d回合结束------
 * 
 ...
 * 
 * \FS[FontSize]
 * 使用drawTextEx的文本(如消息)的字符大小更改为FontSize。
 * 此控制字符与\{和\}不同，没有最小值和最大值。
 * ------------------------------------------------------
 * 使用条款
 * ------------------------------------------------------
 * 本插件已在MIT许可证上发布。
...
(function() {
    ...
    BattleManager.endTurn = function(){
		...
		if ($gameTroop._turnCount - helloCounts > 1) {
			$gameTroop._turnCount--;
		} 
		if (helloCounts < $gameTroop._turnCount) {
			$gameTemp.addLog(text);
		}
		helloCounts = $gameTroop._turnCount;

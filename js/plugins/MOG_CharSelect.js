...
/*:
 * @plugindesc (v1.1)[v1.2]  主菜单 - 角色选择界面
 ...
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 先确保项目img文件夹下是否有charselect文件夹！    (img/charselect)
 * 先确保项目img文件夹下是否有pictures文件夹!       (img/pictures)
 * 如果没有，需要自己建立。需要配置资源文件：
 *
 * 资源-整体布局      (charselect文件夹)
 * 资源-静态背景      (charselect文件夹)
 * 资源-选择指针      (charselect文件夹)
 *
 * 待选角色-1 资源-待选头像      (charselect文件夹)
 * 待选角色-1 资源-选中头像      (charselect文件夹)
 * 待选角色-1 资源-角色前视图    (pictures文件夹)
 *
 * 待选角色-2 资源-待选头像      (charselect文件夹)
 * 待选角色-2 资源-选中头像      (charselect文件夹)
 * 待选角色-2 资源-角色前视图    (pictures文件夹)
 *
 * 待选角色-3 资源-待选头像      (charselect文件夹)
 * ……
 *
 * 角色选择界面有自带的静态背景，所以不支持菜单背景插件。
 *
 * -----------------------------------------------------------------------------
 * ----激活条件
 * 打开角色选择面板，使用插件指令打开：
	...
	for (var i = 0; i < membersTemp.length; i++) {
		 ...
		 for (var j = 1; j <= Moghunter.charSel_char_list_length; j++) {
            ...
            if (actor._actorId == Moghunter.charSel_char_list[j]['待选角色']) {
				...
				this._faceBitmaps.push(ImageManager.loadACharSelect(Moghunter.charSel_char_list[j]['资源-待选头像'] || ""));
				this._faceBitmaps_S.push(ImageManager.loadACharSelect(Moghunter.charSel_char_list[j]['资源-选中头像'] || ""));
				this._pictureBitmaps.push(ImageManager.loadPicture(Moghunter.charSel_char_list[j]['资源-角色前视图'] || ""));
			};
		 };	
	};

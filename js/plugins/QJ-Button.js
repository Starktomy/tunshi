...
//=============================================================================
/*:
 * @plugindesc 虚拟按键脚本[V1.5] 2022-2-16
 * @author Qiu Jiu
 * @help QJ-Button.js
 *
 *====================================
...
getName=(data,x)=>{return "./img/pictures/"+(x?data.name:data.namex)+".png";};
...
const dirButton=QJ.B.loadDetailData(JsonEx.parse(parameters['dirButton']));
const d=[];
...
Game_Temp.prototype.setDestination = function(x, y) {
    if (forBidDestination||forBidButtonDesTemp) {
        forBidButtonDesTemp = false;
        return;
    }
...
DataManager.setupNewGame = function() {
    ...
    if (hasShowOnOpen) {
        hasShowOnOpen = false;
        if (!NBShow) return;
        document.getElementById("ErrorPrinter").style["pointer-events"]="none";
...
NBButton.prototype.setInput = function() {
    if (this.Data.button.length>0) Input._currentState[this.Data.button]=true;
};
NBButton.prototype.clearInput = function() {
    if (this.Data.button.length>0) Input._currentState[this.Data.button]=false;
};

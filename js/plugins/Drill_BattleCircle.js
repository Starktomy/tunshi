 ...
 * -----------------------------------------------------------------------------
 * ----插件说明
 * 1.魔法圈的图片层级将与GIF、背景等相互控制。
 * 2.魔法圈是单个的图片事件，可以伸缩、移动变换。
 * 3.魔法圈不旋转也可以作为一个单独的背景、贴图或者滤镜使用。
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
 ...
 * 参数B：资源文件名
 *        所有资源文件都在battlebacks1下配置，不需要.png后缀。
 * 参数C：图片层级
 ...
 * 示例：
 * 插件指令：>清空战斗魔法圈
 * 插件指令：>创建战斗魔法圈 : 1 : 数字阵列 : 1 : 上层 : -1.0 : 1.0 : 0.60
 * 插件指令：>创建战斗魔法圈 : 2 : F碎片 : 12 : 下层 : 0.93 : 0 : 0.00
 ...
 * 插件指令：>战斗魔法圈 : A : 变色调 : H : I : O1 : O2 : O3 : O4
...
// * 插件指令
//=============================================================================
var _drill_battle_circles_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	_drill_battle_circles_pluginCommand.call(this, command, args);
	
	if (command === '>创建战斗魔法圈') { // >创建战斗魔法圈 : 1 : M数字阵列 : 1 : 上层 : -1.0 : 1.0 : 0.01 : 0.60
		if(args.length == 16){
			var index = Number(args[1]);
			$gameSystem._drill_battle_circles_data[index] = {};								//魔法圈id
			$gameSystem._drill_battle_circles_data[index]['src_img'] = String(args[3]);		//魔法圈资源名
			$gameSystem._drill_battle_circles_data[index]['zIndex'] = Number(args[5]);		//魔法圈图片层级
			$gameSystem._drill_battle_circles_data[index]['area_index'] = String(args[7]);	//魔法圈菜单层级
			$gameSystem._drill_battle_circles_data[index]['x'] = Number(args[9]);			//魔法圈 X
			$gameSystem._drill_battle_circles_data[index]['y'] = Number(args[11]);			//魔法圈 Y
			$gameSystem._drill_battle_circles_data[index]['r_speed'] = Number(args[13]);	//魔法圈 旋转速度
			$gameSystem._drill_battle_circles_data[index]['rate'] = Number(args[15]);		//魔法圈位移比
			$gameSystem._drill_battle_circles_data[index]['x_speed'] = 0;
			$gameSystem._drill_battle_circles_data[index]['y_speed'] = 0;
		}
	}
	if (command === '>清空战斗魔法圈') {
	...
	if (command === '>战斗魔法圈') { // >战斗魔法圈 : A : 变色调 : H : I : J1 : J2 : J3 : J4
		if(args.length >= 10){
			...
			if( SceneManager._scene.constructor.name === "Scene_Battle" ){		//区别战斗外，战斗中的开始时间
				changing['start'] = Number(args[5]) + ($gameSystem._drill_battle_circles_timer || 0);
			}
			$gameSystem._drill_battle_circles_changing.push(changing);
		}
	}
};
//==============================
// * 插件设置存储准备
//==============================
var _drill_battle_circle_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
	_drill_battle_circle_sys_initialize.call(this);
    this._drill_battle_circles_data = [];
    this._drill_battle_circles_changing = [];
    this._drill_battle_circles_timer = 0;
};
...
// ** 菜单后面层（在敌人、角色战斗图之前放置）
//==============================
var _drill_battle_circle_createBattleback = Spriteset_Battle.prototype.createBattleback;
Spriteset_Battle.prototype.createBattleback = function() {    
	_drill_battle_circle_createBattleback.call(this);
...
// * 创建魔法圈
//==============================
var _drill_battle_circle_createLowerLayer = Spriteset_Battle.prototype.createLowerLayer;
Spriteset_Battle.prototype.createLowerLayer = function() {
    _drill_battle_circle_createLowerLayer.call(this);
	
	this._drill_sprite_circles = [];		//数据初始化
	this._drill_sprite_circles_bitmap = [];
	this._drill_sprite_circles_data = [];
	$gameSystem._drill_battle_circles_timer = 0;
	...
	for (var i = 0; i < $gameSystem._drill_battle_circles_data.length; i++) {
		if( $gameSystem._drill_battle_circles_data[i] != null ){
			var temp_sprite_data = JSON.parse(JSON.stringify( $gameSystem._drill_battle_circles_data[i] ));	//拷贝object（杜绝引用造成的修改）
			var temp_sprite_bitmap = new Sprite(ImageManager.loadBattleback1(temp_sprite_data['src_img']));
			...
			this._drill_sprite_circles.push(null);
			this._drill_sprite_circles_bitmap.push(null);
			this._drill_sprite_circles_data.push(null);
		}
	}
	//alert( JSON.stringify( $gameSystem._drill_battle_circles_changing) );
...
// * 刷新魔法圈
//==============================
var _drill_battle_circle_update = Spriteset_Battle.prototype.update;
Spriteset_Battle.prototype.update = function() {
	_drill_battle_circle_update.call(this);
	
	this.drill_updateCircles();
};

Spriteset_Battle.prototype.drill_updateCircles = function() {
	$gameSystem._drill_battle_circles_timer += 1;
	for (var i = 0; i < this._drill_sprite_circles.length; i++) {
		if( this._drill_sprite_circles[i] != null ){
			var temp_sprite = this._drill_sprite_circles[i];
			var temp_sprite_bitmap = this._drill_sprite_circles_bitmap[i];
			var temp_sprite_data = this._drill_sprite_circles_data[i];
			var b_timer = $gameSystem._drill_battle_circles_timer;
			...
			//变化-每帧开始
			for (var j = 0; j < $gameSystem._drill_battle_circles_changing.length; j++) {
				var t = $gameSystem._drill_battle_circles_changing[j];
				if( t['id'] == i ){
			...
			//4.变化-变化结束生命周期
			for(var k = $gameSystem._drill_battle_circles_changing.length -1 ; k >= 0 ; k--){
				var temp_changing = $gameSystem._drill_battle_circles_changing[k];
				if( temp_changing['destroy'] ){
					$gameSystem._drill_battle_circles_changing.splice(k, 1);
				}
			}
		}
	};

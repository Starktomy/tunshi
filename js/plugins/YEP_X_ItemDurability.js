...
Game_Actor.prototype.durabilityBreakItem = function(obj) {
    ...
    var text = this.name()+obj.name+"损坏了";
...
Game_Actor.prototype.damageAllWeaponDurability = function(value, subject) {
    this.damageAllDurability(-1, this.weapons(), subject,"weapons");
};

Game_Actor.prototype.damageAllArmorDurability = function(value, subject) {
    this.damageAllDurability(-1, this.armors(), subject,"armors");
};

Game_Actor.prototype.damageAllDurability = function(value, group, subject, keys) {
    if (value === 0) return;
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (obj.durability < 0 || !$gameParty.inBattle()) continue;
      if (obj.nonIndependent) continue;
 
      var flag = 1;
      for(var ii=0;ii<subject.length;ii++) {
          if (subject[ii] == obj.baseItemId) {
            flag = 2;
            break;
          }
      }
      if (flag == 1) {
        subject.push(obj.baseItemId);
        obj.durability += value;
        if (obj.durability <= 0) removed.push(obj);
      }
    }
    window.localStorage.setItem(keys,subject.join(","));
...
Game_Actor.prototype.damageRandomWeaponDurability = function(value) {
    this.damageRandomDurability(0, this.weapons());
};

Game_Actor.prototype.damageRandomArmorDurability = function(value) {
    this.damageRandomDurability(0, this.armors());
};
...
Game_Action.prototype.applyDurabilityEffects = function(target) {
    var weapons = window.localStorage.getItem("weapons");
    if (weapons) {
    ...
    var armors = window.localStorage.getItem("armors");
    if (armors) {
        armors = armors.split(",");
    } else {
        armors = [];
    ...
    if (this.subject().isActor()) {
      ...
      this.subject().damageAllWeaponDurability(value, weapons);
      ...
      this.subject().damageAllArmorDurability(value, armors);

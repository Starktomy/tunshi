...
//=============================================================================
 /*:
 * @plugindesc v1.09BUG修复版（由darian达达更新） (需要 YEP_ItemCore.js) Allows independent items to
 ...
 *   add slots               - 增加插槽命令，但是需要和Slots: 0配合使用
 *
...
Yanfly.Param.IUSMAXSlots = Number(Yanfly.Parameters['MAX Slots']);
...
ItemManager.initSlotUpgradeNotes = function(item) {
    ...
    //note13用来识别打孔工具。
    var note13 = /(?:ADD SLOTS|add slots)/i;
...
ItemManager.randomizeSlots = function(baseItem, newItem) {
    ...
    if (newItem.upgradeSlots >= Yanfly.Param.IUSMAXSlots) {
        newItem.upgradeSlots = Yanfly.Param.IUSMAXSlots;
    }
...
ItemManager.payIUSEffects = function(mainItem, effectItem) {
	var SlotAttr = window.localStorage.getItem("SlotsAttr");
	if (SlotAttr == "1") {
		attrs = 0;
		var maxForgeCount = mainItem.meta["Upgrade Slots"];
		maxForgeCount = parseFloat(maxForgeCount.trim());
		var tmpCount = mainItem.upgradeSlots+1;
		if (tmpCount > maxForgeCount) {
		} else {
			$gameParty.gainItem(effectItem, 1);
			mainItem.upgradeSlots += effectItem.upgradeSlotCost;
			
			this.decreaseItemBoostCount(mainItem, effectItem.boostCountValue);
			//添加锻造
...
ItemManager.processIUSEffect = function(line, mainItem, effectItem) {
    // 命中
    if (line.match(/((?:(Xiangqian MZ)):[ ]([\+\-]\d+)([%％]))/i)) {
		// ITE1 = mainItem;
		var O = 0;
		for(var i = 0 ;i < mainItem.traits.length;i++){
			if(mainItem.traits[i].code == 22 && mainItem.traits[i].dataId == 0){
			    return mainItem.traits[i].value += Number(String(RegExp.$3)) / 100;
			} else{
				O++
			}
		}
		if(i == O) return mainItem.traits[i] = {code: 22, dataId: 0, value: Number(String(RegExp.$3)) / 100};
	}
	// 暴击
    if (line.match(/((?:(Xiangqian BJ)):[ ]([\+\-]\d+)([%％]))/i)) {
		// ITE1 = mainItem;
		var O = 0;
		for(var i = 0 ;i < mainItem.traits.length;i++){
			if(mainItem.traits[i].code == 22 && mainItem.traits[i].dataId == 2){
			    return mainItem.traits[i].value += Number(String(RegExp.$3)) / 100;
			} else{
				O++
			}
		}
		if(i == O) return mainItem.traits[i] = {code: 22, dataId: 2, value: Number(String(RegExp.$3)) / 100};
	}
	//元素
    if (line.match(/((?:(ELEMENT AMPLIFY))[ ](\d+):[ ]([\+\-]\d+)([%％]))/i)) {
		// ITE1 = mainItem;
		if(!mainItem.elementAmplify[String(RegExp.$3)]) mainItem.elementAmplify[String(RegExp.$3)] = 0;
        return mainItem.elementAmplify[String(RegExp.$3)] += Number(String(RegExp.$4)) / 100;
    }
    if (line.match(/((?:ELEMENT AMPLIFY)[ ](.*):[ ]([\+\-]\d+)([%％]))/i)) {
        if(!mainItem.elementAmplify[String(RegExp.$3)]) mainItem.elementAmplify[String(RegExp.$3)] = 0;
	    return mainItem.elementAmplify[String(RegExp.$3)] += Number(String(RegExp.$4)) / 100;
    }
    ...
    // ADD SLOTS
    if (line.match(/ADD SLOTS/i)) {
      return this.effectIUSAddSlots(mainItem);
    }
};


ItemManager.effectIUSAddSlots = function(mainItem) {
    mainItem.upgradeSlots += 1;
    if (!mainItem.originalUpgradeSlots && mainItem.originalUpgradeSlots != 0) {
      mainItem.originalUpgradeSlots = Yanfly.Param.IUSDefaultSlots;
    }
    mainItem.originalUpgradeSlots += 1;
    mainItem.boostCount -= 1;
    mainItem.slotsApplied.pop();
...
ItemManager.effectIUSRandomChange1 = function(item, stat, value) {
    ...
    switch (stat) {
      ...
      case 'ALL':
		for (var i = 0; i < 8; ++i) {
		  item.params[i] += Math.floor(Math.random() * randomValue - offset)*attrs;
		}
...
ItemManager.effectIUSRandomChange2 = function(item, stat, value) {
    ...
    switch (stat) {
      ...
      case 'ALL':
        for (var i = 0; i < 8; ++i) {
          item.params[i] += Math.floor(Math.random() * randomValue)*attrs;
        }
        break;
      case 'CURRENT':
        for (var i = 0; i < 8; ++i) {
...
ItemManager.effectIUSParamRateChange = function(item, stat, value) {
    ...
    switch (stat) {
      ...
      case 'ALL':
        for (var i = 0; i < 8; ++i) {
          item.params[i] += value * 0.01 * baseItem.params[i]*attrs;
        }
        break;
      case 'CURRENT':
        for (var i = 0; i < 8; ++i) {
...
Window_ItemInfo.prototype.drawSlotsInfo = function(dy) {
	...
	// 居中对齐
    this.drawText(text, dx + 215, dy, dw);
    if (item.originalUpgradeSlots || item.originalUpgradeSlots == 0) {
...
Window_ItemInfo.prototype.drawSlotUpgradesUsed = function(dy) {
    ...
    for (var i = 0; i < item.slotsApplied.length; ++i) {
      ...
      this.drawTextEx(text, dx + 215, dy);
      dy += this.lineHeight();
    }
    this.resetFontSettings();
    return dy;
...
Window_ItemActionCommand.prototype.addUpgradeCommand = function() {
	...
	var text2 = "溶解 \\i[0]\\c[0]"+this._item.name


	this.addCommand(text, 'upgrade', enabled);

    var Slots = window.localStorage.getItem("Slots");
    if (Slots == "1") {
        var meta = this._item.meta
        meta = JSON.stringify(meta);
        if (meta.indexOf("Upgrade") != -1) {
            this.addCommand(text2, 'upgrade', enabled);
        }
    }
};
...
Window_UpgradeItemList.prototype.isEnabled = function(item) {
    ...
    if (this._item.originalUpgradeSlots >= Yanfly.Param.IUSMAXSlots && item.addSlots) {
        return false;
    }
...
Yanfly.IUS.Scene_Item_createActionWindow = Scene_Item.prototype.createActionWindow;
...
Scene_Item.prototype.onActionUpgrade = function() {
	...
	var index = this._itemActionWindow.index();
	var currentText = this._itemActionWindow.commandName(index);
	if(currentText.indexOf("溶解") !== -1){
		window.localStorage.setItem("SlotsAttr","1");
	} else {
		window.localStorage.setItem("SlotsAttr","2");
	}

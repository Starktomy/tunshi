...
Game_BattlerBase.prototype.paySkillCost = function(skill) {
    ...
    var tpcost = window.localStorage.getItem("tpcost");
    if (tpcost == "1") {
        skill.tpCost = 0;
    }
...
Game_Party.prototype.removeActor = function(actorId) {
	var member = $gameActors.actor(actorId);
    if (member) {
		for (var i = 0; i < member._equips.length; i++) {
		  if (member._equips[i]) {
			$gameParty.gainItem(member._equips[i].object(), 1);
			member._equips[i].setObject(null);
		  }
		}
    }
...
Game_Troop.prototype.goldRate = function() {
	var doubleGold = window.localStorage.getItem("doubleGold");
	if (doubleGold) {
		doubleGold = parseInt(doubleGold);
		return doubleGold;
	}
